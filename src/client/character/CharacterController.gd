...
@onready var character: CharacterBody3D = get_parent() as CharacterBody3D
@export var jumping := false

@onready var camera_pivot: CameraController = get_parent().find_child("CameraPivot")

func _enter_tree() -> void:
	# This is a terrible way to do this.
	# But I have no choice since we can't bridge CS to GDScript.
	var peer_id: int = get_parent().name.to_int()
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_released("jump") and character.velocity.y > JUMP_VELOCITY_MIN:
